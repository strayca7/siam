...
func TestFrameMarshalText(t *testing.T) {
	var tests = []struct {
		...
		initpc,
		`^github\.com/strayca7/siam/pkg/serrors\.init(ializers)? .*github\.com/strayca7/siam/pkg/serrors/stack_test\.go:\d+$`,
	}, {
		0,
		`^unknown$`,
	}}
	for i, tt := range tests {
		...
		if !regexp.MustCompile(fmt.Sprintf("%v", tt.want)).Match(got) {
